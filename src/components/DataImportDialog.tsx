...
import type { FieldKey } from '../domain/imports/types';
...
export default function DataImportDialog({ open, onClose, onDone }: Props) {
  const { primaryColor, canRepair, repairDialog, fileRef, fileName, parsed, parsing, busy, progress, report, quality, fixing, prices, showAllRows, setShowAllRows, opts, setOpts, handleFile, runImport, runAutoFix, percent, subjectRows, withoutSubject, visibleRows } = useTableImport(open, onDone);
  return (
    <Modal isOpen={open} onClose={busy || fixing ? () => { } : onClose} title="استيراد داتا (Excel / CSV / JSON)" size="xl">
      {repairDialog}
      <div className="p-5 space-y-4 overflow-y-auto">
        {/* ---------- الملف ---------- */}
        <label
          className={`flex items-center gap-3 px-4 py-3 border-2 border-dashed rounded-xl cursor-pointer transition-colors ${parsed ? 'border-emerald-200 bg-emerald-50' : 'border-gray-200 hover:bg-gray-50'
            }`}
        >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div>
            ...
            <input
              type="number" min={0} step={50} disabled={busy || fixing}
          ...
          <div>
            ...
            <input
              type="number" min={1} disabled={busy || fixing}
        ...
        {quality && (
          <div className="border border-gray-100 rounded-xl p-4 space-y-3">
            ...
            {quality.issues.length === 0 ? (
              ...
              <div className="space-y-1.5 max-h-44 overflow-y-auto">
                {quality.issues.map(issue => (
                  <div key={issue.code}
                    className={`text-xs rounded-lg px-3 py-2 border ${issue.severity === 'error' ? 'bg-red-50 border-red-100 text-red-800'
            ...
            {canRepair && quality.issues.some(i => i.autoFixable) && (
