...
import { fold, guessGender, normalize, normalizePhone } from '../../domain/imports/normalization';
import { DEFAULT_TABLE_IMPORT_OPTIONS } from '../../domain/imports/options';
import type { TableImportOptions, TableImportReport, TableRecord } from '../../domain/imports/types';
...
import { getSubject, subjectPrice, SUBJECTS } from '../../lib/subjects';
...
export function buildGroupName(record: TableRecord, subject: Subject | null): string {
  const parts = [
...
export async function importTableIntoDb(
  ...
  const teacherByPhone = new Map(existingTeachers.map(t => [normalizePhone(t.phone), t] as const).filter(([p]) => !!p));
  ...
  const priceOfSubject = (id: SubjectId): number => subjectPrice(id, opts.subjectPrices);
  ...
  for (const [key, record] of teacherRows) {
    const name = normalize(record.teacherName) || 'غير محدد';
  ...
  for (const record of records) {
    ...
    const price =
      subject && opts.useSubjectPrices ? priceOfSubject(subject.id) : Math.max(0, record.price ?? opts.fallbackPrice);

    // كورس موجود بالاسم أو بالمادة؟
    const existing = record.courseName
      ? courseByName.get(fold(record.courseName))
      : subject
        ? courseBySubject.get(subject.id)
        : undefined;
    ...
    const course: Course = {
      id: generateId(),
      name: normalize(record.courseName) || subject?.name || 'كورس عام',
  ...
  for (const record of records) {
    ...
    const name = normalize(record.groupName) || buildGroupName(record, record.subject);
  ...
  for (const record of records) {
    ...
    const student: Student = {
      ...
      notes:
        record.notes || (anyPhone ? 'مضاف من استيراد داتا' : 'مضاف من استيراد داتا (التليفون placeholder — عدّله)'),
      status: 'active',
      totalPaid: 0,
      enrolledGroups: [],
      createdAt: now,
  ...
  for (const record of records) {
    ...
    const group = await readById<Group>('groups', groupId);
    const course = group ? courseById.get(group.courseId) : undefined;
    const coursePrice = course?.price ?? 0;
    const rowPrice = record.price;
    const needsOverride =
      opts.rowPriceAsOverride && rowPrice !== undefined && rowPrice >= 0 && Math.abs(rowPrice - coursePrice) > 0.009;
  ...
  report.subjectsUsed = SUBJECTS.filter(s => subjectGroupCount.has(s.id)).map(s => ({
    id: s.id,
    name: s.name,
