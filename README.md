...
- **نماذج أجر متعددة**: ثابت / بالحصة / نسبة من المحصّل / لكل مجموعة (تُستخدم في تقرير ربحية المجموعات)
...
| تصفير المديونيات | `writeOffDebts(scope, reason)` في `src/lib/db.ts` + نافذة تأكيد في صفحة المديونيات + حدث `writeoff` في سجل المراجعة |
...
> الجداول `cashbox_sessions / payroll / teacher_advances / message_* / waitlist` موجودة في المخطط (وبتتزامن) للحفاظ على توافق النسخ الاحتياطية، لكن مفيش شاشات تشغيلية ليها في هذه النسخة.
...
**660+ اختباراً** (Vitest + Testing Library + fake-indexeddb) تغطّي منطق الحسابات والنطاقات والطباعة والاستيراد والمزامنة وسجل المراجعة والإيصالات والتحويل والمرتبات والتقرير اليومي والجدول والصلاحيات.
...
│   ├── cashbox.ts     # إجماليات اليوم (بطريقة الدفع/بالموظف) للتقرير اليومي
│   ├── payroll.ts     # نماذج أجر المدرسين وربحية المجموعات
...
│   ├── search.ts      # البحث الشامل من الهيدر
...
├── pages/             # 19 صفحة (Dashboard, Students, StudentProfile, Teachers, TeacherProfile,
│                      #  Courses, Groups, Payments, Debtors, Attendance, Exams, Expenses,
│                      #  Inventory, Reports, DailyReports, Users, Settings, AuditLog, Login)
